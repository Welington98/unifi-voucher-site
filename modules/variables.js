...
module.exports = {
    ...
    authLdapEnabled: config('auth_ldap_enabled') !== null ? config('auth_ldap_enabled') : (process.env.AUTH_LDAP_ENABLED === 'true') || false,
    authLdapUrl: config('auth_ldap_url') || process.env.AUTH_LDAP_URL || 'ldap://localhost:389',
    authLdapBindDn: config('auth_ldap_bind_dn') || process.env.AUTH_LDAP_BIND_DN || '',
    authLdapBindPassword: config('auth_ldap_bind_password') || process.env.AUTH_LDAP_BIND_PASSWORD || '',
    authLdapSearchBase: config('auth_ldap_search_base') || process.env.AUTH_LDAP_SEARCH_BASE || '',
    authLdapSearchFilter: config('auth_ldap_search_filter') || process.env.AUTH_LDAP_SEARCH_FILTER || '(sAMAccountName={{username}})',
    authLdapTlsSkipVerify: config('auth_ldap_tls_skip_verify') !== null ? config('auth_ldap_tls_skip_verify') : (process.env.AUTH_LDAP_TLS_SKIP_VERIFY === 'true') || false,
    authLdapAllowedUsers: config('auth_ldap_allowed_users') || process.env.AUTH_LDAP_ALLOWED_USERS || '',
    ...
    kioskAdminPassword: config('kiosk_admin_password') || process.env.KIOSK_ADMIN_PASSWORD || '',
